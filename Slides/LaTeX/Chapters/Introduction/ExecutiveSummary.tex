% ------------------------------------------------------------------------------
% TYPO3 Version 11.3 - What's New (English Version)
%
% @author	Michael Schams <schams.net>
% @license	Creative Commons BY-NC-SA 3.0
% @link		https://typo3.org/help/documentation/whats-new/
% @language	English
% ------------------------------------------------------------------------------
% TYPO3 Version 11.3 - Executive Summary

\begin{frame}[fragile]
	\frametitle{Introduction}
	\framesubtitle{Executive Summary}

	\small
		...
		\vspace{0.2cm}

		TYPO3 version 11.3 is the fourth sprint release on the way to the LTS-version
		(long-term support) in 2021.

		\vspace{0.2cm}

		In addition to version 7.4, the TYPO3 Core now also officially supports
		PHP version 8.0. Developers can leverage many new features, optimizations,
		and improvements of the programming language that powers TYPO3.

	\normalsize

\end{frame}

% ------------------------------------------------------------------------------
