% ------------------------------------------------------------------------------
% TYPO3 Version 11.3 - What's New (English Version)
%
% @author	Michael Schams <schams.net>
% @license	Creative Commons BY-NC-SA 3.0
% @link		https://typo3.org/help/documentation/whats-new/
% @language	English
% ------------------------------------------------------------------------------
% Feature | 93921 | Sharing backend links

\begin{frame}[fragile]
	\frametitle{Backend User Interface}
	\framesubtitle{Sharing Backend Links}

	Sharing TYPO3 backend deeplinks is even easier now. Backend users can copy the
	URL of the page to their clipboard with a click of a button.

	\begin{figure}
		\includegraphics[width=0.80\linewidth]{BackendUserInterface/1624461728-CopyDeepLinksToClipboard}
	\end{figure}

\end{frame}

% ------------------------------------------------------------------------------
