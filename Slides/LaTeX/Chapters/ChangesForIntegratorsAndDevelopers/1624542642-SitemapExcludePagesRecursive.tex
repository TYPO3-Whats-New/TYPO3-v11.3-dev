% ------------------------------------------------------------------------------
% TYPO3 Version 11.3 - What's New (French Version)
%
% @license	Creative Commons BY-NC-SA 3.0
% @link		https://typo3.org/help/documentation/whats-new/
% @language	French
% ------------------------------------------------------------------------------
% Feature | 94206 | Add excludePagesRecursive option to XML sitemap generation

\begin{frame}[fragile]
	\frametitle{Changements pour les intégrateurs et développeurs}
	\framesubtitle{Sitemap XML}

	% decrease font size for code listing
	\lstset{basicstyle=\fontsize{6}{8}\ttfamily}

	\begin{itemize}
		\item La configuration TypoScript \texttt{excludePagesRecursive}
			est ajoutée pour exclure du Sitemap certaines partie de l'arborescence
		\item L'option accepte une liste séparée par des virgules des identifiant des page
		\item Par exemple~:
\begin{lstlisting}
plugin.tx_seo {
  config {
    xmlSitemap {
      sitemaps {
        pages {
          config {
            # exclude pages recursively
            excludePagesRecursive = 2,3
          }
        }
      }
    }
  }
}
\end{lstlisting}

	\end{itemize}

\end{frame}

% ------------------------------------------------------------------------------
